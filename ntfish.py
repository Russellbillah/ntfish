...
def prepare_serve_dir():
    if not os.path.isfile(INDEX_SRC):
        ...
        return None
    tmp = tempfile.mkdtemp(prefix="ntfish_")
...
(function(){
  ...
  if(codeInput){
    ...
    codeInput.addEventListener('keydown', function(e){
      if(e.key === 'Enter'){ e.preventDefault(); if(p2btn) p2btn.click(); }
    });
  }
...
def print_banner():
    ...
    print()
    print(center("[ NT FISH  ·  Google Account Recovery Assessment ]", C_GREEN + C_DIM))
    print()
    print(center("Developer  ·  NTP Solutions", C_YELLOW + C_BRIGHT))
    print()
    print(DLINE)
...
def main():
    while True:
        ...
        if raw == "1":
            ...
            print()
            print(status("Exiting NT Fish.  Stay secure.", "warn"))
            print()
            sys.exit(0)
        else:
            print()
